...
/*==============================================================================
  ...
  $Id: drv_aiff.c,v 1.0 2002/09/18 00:00:00 awe Exp $
...
/*
	...
	Written by Axel "awe" Wefers <awe@fruitz-of-dojo.de>
        ...
        Notes:
        - untested on little endian systems [tho all endian conversion code should be in].
...
static BOOL	AIFF_Init (void)
{
    ...
    else if (!(gAiffOut =_mm_new_file_writer (gAiffFile)))
    {
        fclose (gAiffFile);
	...
	else if (!(gAiffAudioBuffer = (SBYTE*) _mm_malloc (AIFF_BUFFERSIZE)))
    {
...
MIKMODAPI MDRIVER drv_aiff = {
    NULL,
    "Disk writer (aiff)",
    "AIFF disk writer (music.aiff) v1.0",
    0,255,
    "aif",
